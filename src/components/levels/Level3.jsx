...
const Level3 = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    let player, enemies, artifacts, courageOrbs, walls, templeGates, templeDoors;
    ...
    function preload() {
      ...
      const enemyTypes = ['skeleton_warrior', 'stone_golem', 'shadow_guardian', 'ancient_mummy', 'demon_guardian'];
      ...
      const artifactTypes = ['golden_idol', 'crystal_orb', 'ancient_scroll', 'mystic_amulet', 'sacred_chalice'];
      ...
      this.add.graphics().fillStyle(0x8b4513).fillRect(0, 0, 40, 40).generateTexture('temple_wall', 40, 40);
      this.add.graphics().fillStyle(0x654321).fillRect(0, 0, 800, 500).generateTexture('temple_background', 800, 500);
    ...
    function create() {
      ...
      this.physics.add.overlap(player, courageOrbs, collectHealingOrb, null, this);
    ...
    function createTempleWalls() {
      ...
      const wallPositions = [
        ...
        // Cross passages
        [280, 200], [320, 200], [480, 200], [520, 200],
        [280, 300], [320, 300], [480, 300], [520, 300]
      ];
      
      wallPositions.forEach(pos => walls.create(pos[0], pos[1], 'temple_wall'));
    }
    ...
    function spawnArtifacts() {
      ...
      const artifactPositions = [
        ...
        { x: 550, y: 160, type: 'ancient_scroll', id: 3, name: 'Ancient Scroll' },
        { x: 250, y: 340, type: 'mystic_amulet', id: 4, name: 'Mystic Amulet' },
        { x: 550, y: 340, type: 'sacred_chalice', id: 5, name: 'Sacred Chalice' }
      ];
      
      artifactPositions.forEach(data => {
        const artifact = artifacts.create(data.x, data.y, data.type);
        artifact.setCollideWorldBounds(true).body.setSize(25, 30).setOffset(3, 3);
      ...
      const messageText = sceneRef.add.text(400, 100, '✨ Ancient Artifacts Have Appeared! ✨\nDefeat all guardians, then write the SQL query to open the temple!', {
        fontSize: '16px',
    ...
    function update() {
      ...
      if (!gameState.allEnemiesKilled && gameState.enemiesDefeated >= gameState.totalEnemies) {
        gameState.allEnemiesKilled = true;
        spawnArtifacts();
        
        // Show query input after a delay
        sceneRef.time.delayedCall(2000, () => {
          setUiState(prev => ({ ...prev, showQueryInput: true, allEnemiesKilled: true }));
        });
      }
    }
      ...
      enemies.children.entries.forEach(enemy => {
        ...
        if (distance <= attackRange) {
          ...
          if (enemy.health <= 0) {
            ...
            const explosion = sceneRef.add.circle(enemy.x, enemy.y, 30, 0xff6b6b);
            sceneRef.tweens.add({
              targets: explosion,
              scaleX: 4,
              scaleY: 4,
    ...
    function collectArtifact(player, artifact) {
      if (!gameState.templeDoorsOpen) {
        ...
        const messageText = sceneRef.add.text(artifact.x, artifact.y - 50, 'Open the temple doors first by writing the SQL query!', {
          fontSize: '12px',
          fontFamily: 'Courier New',
          color: '#ff4444',
          backgroundColor: '#000000',
      ...
      if (artifact.artifactData) {
        ...
        const artifactIndex = gameState.artifactsData.findIndex(a => a.id === artifact.artifactData.id);
        if (artifactIndex !== -1) {
      ...
      const collectEffect = sceneRef.add.circle(artifact.x, artifact.y, 40, 0xffd700, 0.7);
      sceneRef.tweens.add({
    ...
    function collectHealingOrb(player, orb) {
      ...
      // Visual effect
      const healEffect = sceneRef.add.circle(player.x, player.y, 30, 0x00ff00, 0.7);
      sceneRef.tweens.add({
        targets: healEffect,
        scaleX: 2,
        scaleY: 2,
    ...
    function openTempleDoors() {
      ...
      // Door opening effect
      const doorEffect = sceneRef.add.circle(templeDoors.x, templeDoors.y, 50, 0xffd700, 0.5);
      sceneRef.tweens.add({
        targets: doorEffect,
        scaleX: 3,
        scaleY: 3,
      ...
      // Show message
      const messageText = sceneRef.add.text(templeDoors.x, templeDoors.y - 80, '🏛️ Temple Doors Opened! 🏛️\nNow collect all the artifacts!', {
        fontSize: '14px',
        fontFamily: 'Courier New',
        color: '#ffd700',
        backgroundColor: '#000000',
    ...
    function enterTemple(player, gate) {
      if (!gameState.templeDoorsOpen) {
        const messageText = sceneRef.add.text(gate.x, gate.y - 50, 'The temple doors are sealed.\nDefeat all guardians and write the SQL query!', {
          fontSize: '12px',
          fontFamily: 'Courier New',
          color: '#ff4444',
          backgroundColor: '#000000',
    ...
    function showLevelComplete() {
      ...
      const completionText = sceneRef.add.text(400, 120, '🏛️ All Artifacts Collected! 🏛️', {
        fontSize: '28px',
        fontFamily: 'Courier New',
        color: '#ffd700',
        fontStyle: 'bold'
      ...
      const queryText = sceneRef.add.text(400, 160, 'Query Executed Successfully:', {
        fontSize: '16px',
        fontFamily: 'Courier New',
        color: '#00ffff'
      }).setOrigin(0.5).setDepth(1001);
      
      const queryText2 = sceneRef.add.text(400, 180, 'SELECT * FROM artifacts WHERE found_by IS NOT NULL;', {
        fontSize: '14px',
        fontFamily: 'Courier New',
        color: '#00ffff',
        fontStyle: 'bold'
      ...
      // Show results
      const resultsText = sceneRef.add.text(400, 220, 'Query Results:', {
        fontSize: '14px',
        fontFamily: 'Courier New',
        color: '#ffff00'
      }).setOrigin(0.5).setDepth(1001);
      ...
      const artifactResults = sceneRef.add.text(400, 280, resultsList, {
        fontSize: '12px',
        fontFamily: 'Courier New',
        color: '#90ee90',
        align: 'center'
      ...
      const statsText = sceneRef.add.text(400, 350, `🗡️ Guardians Defeated: ${gameState.enemiesDefeated}/${gameState.totalEnemies}\n✨ Artifacts Collected: ${gameState.artifactsCollected}/${gameState.totalArtifacts}`, {
        fontSize: '14px',
        fontFamily: 'Courier New',
        color: '#ffff00',
        align: 'center'
    ...
    function hitByEnemy(player, enemy) {
      if (enemy.lastAttack && sceneRef.time.now - enemy.lastAttack < 1500) return;
    ...
    function restartLevel() {
      const restartText = sceneRef.add.text(400, 250, 'The temple guardians have defeated you... Try Again!', {
        fontSize: '20px',
        fontFamily: 'Courier New',
        color: '#ff4444',
        backgroundColor: '#000000',
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4 text-white">
      ...
      <div className="w-full max-w-3xl grid grid-cols-2 gap-4 pixel-font text-sm">
        ...
        <div>Guardians Defeated: <span className="text-red-400">{uiState.enemiesDefeated}/{uiState.totalEnemies}</span></div>
        <div>Artifacts Collected: <span className="text-yellow-400">{uiState.artifactsCollected}/{uiState.totalArtifacts}</span></div>
        <div>Temple Status: <span className={uiState.templeDoorsOpen ? "text-green-400" : "text-red-400"}>
