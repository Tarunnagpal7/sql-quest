...
const Level2 = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    function spawnKeyword() {
      ...
      const needsCorrectKeyword =
        gameState.collectedParts.length < gameState.expectedParts.length;
      ...
      if (shouldSpawnCorrect) {
        ...
        keywordText =
          gameState.wrongKeywords[
            Phaser.Math.Between(0, gameState.wrongKeywords.length - 1)
          ];
      }

      const fishType =
        gameState.fishTextures[
          Phaser.Math.Between(0, gameState.fishTextures.length - 1)
        ];
      ...
      const text = sceneRef.add
        .text(fish.x, fish.y - 25, keywordText, {
          fontSize: "14px",
    ...
    function shootProjectile() {
      ...
      const projectile = sceneRef.add.sprite(
        player.x,
        player.y - 10,
        "projectile"
      );
    ...
    function update() {
      ...
      if (
        cursors.left.isDown ||
        wasdKeys.A.isDown ||
        mobileControlsRef.current.left
      ) {
        player.setVelocityX(-speed);
      } else if (
        cursors.right.isDown ||
        wasdKeys.D.isDown ||
        mobileControlsRef.current.right
      ) {
        player.setVelocityX(speed);
      }
            ...
            if (
              ...
              if (
                ...
                const expectedKeyword =
                  gameState.expectedParts[gameState.collectedParts.length];
    ...
    function showLevelComplete() {
      if (gameState.spawnEvent) {
        gameState.spawnEvent.remove();
      }
      ...
      const completionText = sceneRef.add
        .text(400, 180, "🌿 Jungle River Conquered! 🌿", {
          fontSize: "28px",
      ...
      const instructionText = sceneRef.add
        .text(400, 320, "Tap anywhere to continue your jungle adventure", {
          fontSize: "18px",
          fontFamily: "Courier New",
          color: "#ffffff",
        })
    ...
    <div className="w-full flex flex-col items-center gap-4 text-white">
      <div className="text-center mb-2">
        <p className="text-sm text-slate-400">
          Help the wizard shoot magic spells at the correct SQL keywords
          floating on the fish!
        </p>
      </div>
      ...
      <div className="w-full max-w-3xl p-4 bg-gradient-to-br from-slate-800/80 to-slate-900/80 rounded-lg border border-green-500/50 text-center backdrop-blur-sm">
        <div className="pixel-font text-green-300 mb-3 text-lg">
        ...
        <div className="font-mono text-xl bg-black/40 rounded-lg p-4 border border-slate-600">
          ...
          <span className="text-green-300">jungle_explorers</span>{" "}
        ...
        {uiState.isQueryComplete && (
          <div className="mt-4 text-green-400 text-lg bg-green-900/30 rounded-lg p-3 border border-green-500">
            ✅ Perfect! This query finds brave jungle explorers with courage
