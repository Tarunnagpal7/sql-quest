...
const Level6 = ({ onComplete }) => {
  ...
  const correctQueries = [
    ...
    "select name, skill from jungle_explorers where name = '%a%' and (skill='magic' or skill='healing')",
  ];
  ...
  const handleQuerySubmit = () => {
    ...
    if (isCorrect) {
      ...
      setQueryError(
        'Query failed! Find explorers whose names contain "a" and have magic or healing skills.'
      );
  ...
  useEffect(() => {
    ...
    function preload() {
      ...
      gameState.explorerData.forEach((explorer) => {
        if (explorer.hostile) {
          ...
          enemyGraphics.generateTexture(
            `hostile_${explorer.name.toLowerCase()}`,
            40,
            45
          );
      ...
      gameState.explorerData.forEach((explorer) => {
        if (!explorer.hostile) {
          ...
          hologramGraphics.generateTexture(
      ...
      this.add
        ...
        .generateTexture("cave_background", 800, 500);
    }
    ...
    function spawnExplorers() {
      ...
      explorerPositions.forEach((pos, index) => {
        const explorerData = gameState.explorerData.find(
          (e) => e.name === pos.explorer
        );
        ...
        if (explorerData.hostile) {
          ...
          const enemy = enemies.create(
            pos.x,
            pos.y,
            `hostile_${explorerData.name.toLowerCase()}`
          );
          enemy
          ...
          const nameText = sceneRef.add
            .text(pos.x, pos.y - 40, pos.explorer, {
              fontSize: "12px",
          ...
          const fragment = crystalFragments.create(
            pos.x,
            pos.y,
            `fragment_${explorerData.skill}`
          );
          fragment
            .setCollideWorldBounds(true)
            .body.setSize(25, 25)
          ...
          // Skill text
          const skillText = sceneRef.add
            .text(pos.x, pos.y - 25, explorerData.skill, {
              fontSize: "10px",
              fontFamily: "Courier New",
              color: "#ffffff",
              backgroundColor: "#000000",
    ...
    function createCrystalPowerDisplay() {
      ...
      const powerValueText = sceneRef.add
        .text(120, 60, "", {
          fontSize: "12px",
          fontFamily: "Courier New",
          color: "#ffffff",
    ...
    function update() {
      ...
      if (
        (Phaser.Input.Keyboard.JustDown(interactKey) ||
          mobileControls.interact) &&
        gameState.canInteract
      ) {
        // Check if near altar
        const distanceToAltar = Phaser.Math.Distance.Between(
          player.x,
          player.y,
          crystalAltar.x,
          crystalAltar.y
      ...
      enemies.children.entries.forEach((enemy) => {
        ...
        if (distanceToPlayer < enemy.aggroRange) {
          ...
          if (
            ...
            // Show damage effect
            showMessage(
              `${enemy.explorerData.name} attacks! -${enemy.attackDamage} health`,
              1500
            );
      ...
      enemies.children.entries.forEach((enemy) => {
        ...
        if (distance <= attackRange) {
          ...
          if (enemy.health <= 0) {
            ...
            showMessage(
              `${enemy.explorerData.name} defeated! Fragment dropped!`,
              2000
            );
          }
        }
    ...
    function dropEnemyFragment(enemy) {
      ...
      const fragment = crystalFragments.create(
        enemy.x,
        enemy.y,
        `fragment_${enemy.explorerData.skill}`
      );
    ...
    function collectFragment(player, fragment) {
      ...
      if (fragment.explorerData.isMatch) {
        ...
        showMessage(
          `${fragment.explorerName} fragment collected! Perfect match! +20 Crystal Power`,
          2000
        );
      } else {
        gameState.crystalPower += 10;
        showMessage(
          `${fragment.explorerName} fragment collected. Has 'a' but wrong skill. +10 Crystal Power`,
          2000
        );
      }

      gameState.crystalPower = Math.min(
        gameState.maxCrystalPower,
        gameState.crystalPower
      );
    ...
    function spawnHologram(explorerName, x, y) {
      const hologram = sceneRef.add.sprite(
        x,
        y,
        `hologram_${explorerName.toLowerCase()}`
      );
      hologram.setAlpha(0);
    ...
    function hitByEnemy(player, enemy) {
      if (enemy.lastPlayerHit && sceneRef.time.now - enemy.lastPlayerHit < 1500)
        return;
    ...
    function interactWithAltar() {
      if (gameState.fragmentsCollected < gameState.totalFragments) {
        showMessage(
          `Collect all ${gameState.totalFragments} crystal fragments first! (${gameState.fragmentsCollected}/${gameState.totalFragments})`,
          2500
        );
        return;
      }
    ...
    function completeQuery() {
      ...
      showMessage(
        "Query executed successfully! The crystal fragments resonate with your knowledge!",
        3000
      );
    ...
    function restoreMemoryCrystal() {
      ...
      const restorationEffect = sceneRef.add.circle(
        crystalAltar.x,
        crystalAltar.y,
        100,
        0x9370db,
    ...
    function showLevelComplete() {
      ...
      const completionText = sceneRef.add
        .text(400, 80, "🔮 Memory Crystal Restored! 🔮", {
          fontSize: "28px",
          fontFamily: "Courier New",
          color: "#9370db",
          fontStyle: "bold",
      ...
      const queryText = sceneRef.add
        .text(400, 120, "Query Executed Successfully:", {
          fontSize: "16px",
          fontFamily: "Courier New",
          color: "#00ffff",
        })
      ...
      const resultText = sceneRef.add
        .text(
          400,
          170,
          'Matching Explorers Found (names with "a" AND magic/healing):',
          {
            fontSize: "14px",
            fontFamily: "Courier New",
            color: "#ffff00",
      ...
      const explorerResults = sceneRef.add
        .text(400, 210, resultsList, {
          fontSize: "14px",
          fontFamily: "Courier New",
          color: "#90ee90",
          align: "center",
      ...
      const logicText = sceneRef.add
        .text(400, 270, 'Names WITHOUT "a" were hostile enemies:', {
          fontSize: "12px",
          fontFamily: "Courier New",
          color: "#ff8888",
        })
      ...
      const hostileList = gameState.explorerData
        .filter((e) => e.hostile)
        .map((e) => e.name)
        .join(", ");
      const hostileText = sceneRef.add
        .text(400, 290, hostileList, {
          fontSize: "12px",
          fontFamily: "Courier New",
          color: "#ff4444",
          align: "center",
      ...
      const instructionText = sceneRef.add
        .text(
          400,
          420,
          "The ancient memories are restored! Click to return to map",
          {
            fontSize: "16px",
            fontFamily: "Courier New",
            color: "#00ff00",
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4 text-white">
      ...
      <div className="w-full max-w-3xl grid grid-cols-2 gap-4 pixel-font text-sm">
        ...
        <div>
          Crystal Power:{" "}
          <span className="text-purple-400">{uiState.crystalPower}%</span>
        </div>
        <div>
          Fragments:{" "}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-slate-800 p-6 rounded-lg border border-slate-600 max-w-lg w-full mx-4">
            ...
            <p className="text-slate-300 mb-4 text-sm text-center">
              Write the SQL query to find the correct souls for crystal
              restoration:
            </p>

            <div className="bg-black p-3 rounded border mb-4">
              <p className="text-green-400 text-xs font-mono">
                Find name and skills from jungle_explorers whose names contain
                'a' AND have either 'magic' or 'healing' skills
              </p>
            </div>
